...
function CoAP(opts, done) {
  ...
  this._broker = opts.sgs.broker;
  this._sgs = opts.sgs;

  var logger = this._logger = opts.sgs.logger.child({ service: 'CoAP' });
...
CoAP.prototype._handlePUT = function(topic, req, res) {
  ...
  req.pipe(callback(function(err, payload) {
    ...
    // Receiving msg from COAP
    payload = payload.toString() + " annotated(COAP)";
    console.log(payload);
    ...
    that._persistence.storeRetained(packet, function() {
      that._broker.publish(topic, payload, {}, function() {
        ...
        that._sgs.emit('updated', topic, payload);
      });
    });
  }));
};
